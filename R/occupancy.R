...
occupancy <- function(imgs,focal.channel,target.channel,size,npixel,R=10,dstep=1,pwidth,zstep,cores=1,kern.smooth=NULL,layers=NULL,naming=NULL){
  ...
  OCC.all <- list()
  for(r in 1:R){
    OCC <- occupancy.default(imgs,focal.channel,target.channel,size,npixel,dstep,pwidth,zstep,cores,kern.smootL,layers,naming)
    OCC$R <- r
    OCC.all[[r]] <- OCC
    # Update progress bar
...
occupancy.default <- function(imgs,focal.channel,target.channel,size,npixel,dstep=1,pwidth,zstep,cores=1,kern.smooth=NULL,layers=NULL,naming=NULL) {
